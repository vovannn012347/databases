...
def edit_record(number, title, book_id, reader_id, librarian_id, issue_date, repay_date, real_repay_date):
    ...
    cache.delete(record['librarian']['name'])
    cache.delete(record['reader']['name'])
...
def search_records_by_lib(name):
    if cache.exists(name):
        return pickle.loads(cache.get(name))
    records = list(db.journal.find({'librarian.name': name}))
    if len(records) == 0:
        return None
    cache.set(name, pickle.dumps(records))

    return records


def search_records_by_reader(name):
    if cache.exists(name):
        return pickle.loads(cache.get(name))
    records = list(db.journal.find({'reader.name': name}))
    if len(records) == 0:
        return None
    cache.set(name, pickle.dumps(records))
    return records
